...
**コラージュ**
- [ ] 全セルでタップ→トリミング（開閉スムーズ・見切れなし・パン/ピンチ/リセット）
- [ ] 長押しドラッグで任意のセル入替（ターゲット枠が出る）
- [ ] テンプレ5種 / レイアウト / パレット5配置＋帯調整 / HEXチップ / 色相整列
...
- スクリーンショット（**6.9" と 6.5"** が必須。iPhone 16 Pro Max / 15 Plus シミュレータで ⌘S）
  - 構成案（5枚・全ロケール共通画像＋ローカライズ見出しテキスト）:
    1. ハント「色で、写真が整う。」（コレクションカードが見える状態）
    2. コラージュ組写「HEXパレットで、組写を。」
    3. テンプレカード「ワンタップで雑誌みたいに」
    4. 今日の色「今日は何色をハントする？」
    5. カラーマップ「あなたの街が、パレットになる。」
