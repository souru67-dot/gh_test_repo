...
**写真のメモリ設計**: 常駐するのは `HuntPhoto.thumb`（長辺440px・約0.55MB）だけ。
原寸は `Documents/HuntStore/<id>.jpg` にあり、コラージュのセルとトリミング
エディタが `AppState.fullImage(for:)` で都度読む（NSCache・上限200MB なので
メモリ逼迫時は OS が捨てられる）。

以前は全枚数を原寸のまま `UIImage` で抱えていて、**200枚で実測718MB**。しかも
取り込み経路が2つあり、重さが違った——`自動で仕分け` は1200pxを要求するが、
`写真を選ぶ` は原寸をそのまま保持し、保存が最大2400pxなので**再起動すると4倍で
戻ってきた**。取り込み中のピークも同じで、200枚を配列に貯めてから渡していたため
820MB。いまは3点で閉じている。

- **常駐** … 復元も `downsampled(contentsOf:maxSide:)` で ImageIO に直接その
  サイズでデコードさせる。原寸を1度も展開しない。
- **取り込みピーク** … `collectRecent`（自動仕分け）と `addOne`（ピッカー）が
  1枚ずつ「読む→ハッシュ→サムネ→JPEG書き出し→解放」を回す。同時に生きる
  フル画像は**常に1枚**。
- **書き出し品質** … `canvas(width:images:)` は読み込み済みの原寸を明示的に
  受け取る。`@State` の反映タイミングに依存すると、書き出しが黙って440pxの
  サムネから作られてしまうため。
