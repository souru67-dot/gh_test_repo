...
| VoiceOver は対応済み・Dynamic Type は未検証 | アイコンボタン・スライダー・写真タイル・カラーホイール（`accessibilityAdjustableAction` で15°刻み）にラベルを入れ、コラージュのプレビューは1要素に畳んだ。文字サイズ最大での崩れは未確認 | 実機の最大文字サイズで1周 |
