...
### ⚠️ 貼り付けるときの注意（2026-08-05 に実際に踏んだ2件）

**1. 表の中の値は、前後のバッククォート `` ` `` を含めないでください。**
Markdown のコード装飾です。App名・サブタイトル・キーワードが該当します。
キーワードに `` ` `` が入ると文字数を2つ消費し、検索にも悪影響が出ます。

**2. 罫線文字（`─` U+2500）は App Store Connect が受け付けません。**
「このフィールドには1つ以上の無効な文字が含まれています」になります。
**このドキュメントからは削除済み**（4言語とも）ですが、装飾を足すときは
罫線・矢印・絵文字を説明文に入れないでください。

> `■`（U+25A0）は日本の App Store で広く使われており、問題ありません。
> `＆` `／` などの全角記号も通ります。
